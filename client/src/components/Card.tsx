...
export function Card({ card, selected, onClick, small, faceDown, className = '', style, title }: Props) {
  const dim = small
    ? { w: 'w-20', h: 'h-28', rank: 'text-xl', cornerSuit: 14, suit: 38, topPad: 'top-1.5 left-2' }
    : { w: 'w-24', h: 'h-32', rank: 'text-3xl', cornerSuit: 18, suit: 56, topPad: 'top-1.5 left-2.5' };
  ...
  return (
    <button
      type="button"
      title={title ?? `${card.rank}${card.suit}`}
      onClick={onClick}
      style={style}
    ...
    >
      <div className={`absolute ${dim.topPad} leading-none flex flex-col items-center`}>
        ...
        <SuitIcon suit={card.suit} size={dim.cornerSuit} className="mt-[1px]" />
      </div>
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <SuitIcon suit={card.suit} size={dim.suit} className="drop-shadow-sm" />
      </div>
    </button>
  );
}
